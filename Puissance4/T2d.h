...
void set(const T2d*, char col, unsigned int lig, char symbole);
...
char choixJoueur(const T2d*);// retourne la colonne choisie
int viderBuffer();
bool estRemplie(const T2d*, char col);
unsigned int placerJeton(const T2d*, char col, char symbole); // retourne la ligne dans laquelle le jeton est placée
bool grillePLeine(T2d*);

/* FONCTIONS POUR L'IA*/
char* coupsLegaux(const T2d*); // retourne la liste des coups légaux pour un état du jeu
unsigned int trouverLigne(const T2d* t, char col); // trouve la ligne dans laquelle placer le jeton																		
